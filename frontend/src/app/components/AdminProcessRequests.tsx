...
export function AdminProcessRequests() {
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="payments">
      ...
      <TabsContent value="users" className="mt-4"><Card><CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b bg-gray-50"><th className="p-3 text-left">ID</th><th className="p-3 text-left">Tên đăng nhập</th><th className="p-3 text-left">Họ tên</th><th className="p-3 text-left">Email</th><th className="p-3 text-left">Role</th><th className="p-3 text-left">Thao tác Wallet</th></tr></thead><tbody>{users.map(user => <tr key={user.userId} className="border-b"><td className="p-3">{user.userId}</td><td className="p-3">{user.userName}</td><td className="p-3">{user.fullName || "-"}</td><td className="p-3">{user.email}</td><td className="p-3">{user.roleName || "-"}</td><td className="p-3"><Button size="sm" variant="outline" disabled={busy===`u-${user.userId}`} onClick={() => { const amount=Number(window.prompt("Số tiền top-up:","10000")); if(amount>0) void run(`u-${user.userId}`,()=>adminApi.topup(user.userId,amount,"Admin top-up từ giao diện"),"Đã top-up ví"); }}>Top-up</Button></td></tr>)}</tbody></table>{users.length===0&&<div className="py-12 text-center text-gray-500">Không có dữ liệu user.</div>}</CardContent></Card><Alert className="mt-4"><AlertDescription>UserService hiện đã trả role cho danh sách user. Backend vẫn chưa có API khóa/mở tài khoản, nên frontend chỉ top-up qua WalletService.</AlertDescription></Alert></TabsContent>
    </Tabs>
  </div>;
}
