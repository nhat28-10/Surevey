...
export function HelperMarketplace() {
  ...
  const load = useCallback(async () => {
    ...
    try { setCampaigns(await campaignApi.available()); }
  ...
  return <div className="space-y-6">
    <div><h1 className="text-3xl font-bold">Marketplace khảo sát</h1><p className="text-gray-600 mt-1">Chỉ hiển thị campaign ACTIVE còn hạn và còn chỗ.</p></div>
    <div className="relative max-w-xl"><Search className="absolute left-3 top-3 w-4 h-4 text-gray-400" /><Input value={query} onChange={e => setQuery(e.target.value)} placeholder="Tìm theo tiêu đề, mô tả hoặc danh mục" className="pl-9" /></div>
    {error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
    {loading ? <div className="py-16 text-center text-gray-600">Đang tải campaign...</div> : filtered.length === 0 ?
      <Card><CardContent className="py-16 text-center text-gray-600">Hiện chưa có campaign phù hợp.</CardContent></Card> :
