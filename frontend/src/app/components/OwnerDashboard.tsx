    ...
    <Dialog open={paymentDialogOpen} onOpenChange={setPaymentDialogOpen}>
      <DialogContent className="sm:max-w-2xl">
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={() => paymentInfo && void campaignApi.payment(paymentInfo.id).then(updated => {
            setPaymentInfo(updated);
            if (updated.status === "PAID") {
              setPaymentDialogOpen(false);
              toast.success("Thanh toán đã được xác nhận.");
            }
          }).then(() => load())} className="bg-green-600 hover:bg-green-700">Kiểm tra trạng thái</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>;
...
function CampaignProgressOverview({
  campaigns,
  completedCampaigns,
  paidCampaigns,
  paidBudget,
  ...
  const trackedCampaigns = campaigns
    ...
    .slice()
    .sort((a, b) => {
      const aComplete = a.targetResponses > 0 && a.approvedResponses >= a.targetResponses;
      const bComplete = b.targetResponses > 0 && b.approvedResponses >= b.targetResponses;
    ...
    <CardContent className="space-y-5 pt-5">
      {trackedCampaigns.length === 0 ? <div className="rounded-md border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
        ...
        {trackedCampaigns.map(campaign => {
          const progress = campaign.targetResponses ? Math.min(100, campaign.approvedResponses / campaign.targetResponses * 100) : 0;
          const remaining = Math.max(campaign.targetResponses - campaign.approvedResponses, 0);
          const complete = campaign.targetResponses > 0 && remaining === 0;

          return <div key={campaign.id} className="rounded-md border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 flex items-center gap-3">
              ...
              <span className="w-12 text-right text-sm font-semibold text-slate-700">{Math.round(progress)}%</span>
            </div>
          </div>;
        })}
      </div>}
